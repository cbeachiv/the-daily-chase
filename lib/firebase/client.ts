"use client";

import { getApp, getApps, initializeApp } from "firebase/app";
import { getAuth, type Auth } from "firebase/auth";
import { getFirestore, type Firestore } from "firebase/firestore";

const firebaseConfig = {
  apiKey: process.env.NEXT_PUBLIC_FIREBASE_API_KEY,
  authDomain: process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN,
  projectId: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID,
  storageBucket: process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET,
  messagingSenderId: process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID,
  appId: process.env.NEXT_PUBLIC_FIREBASE_APP_ID,
};

// Firebase is only ever touched in browser-side effects and event handlers.
// Initializing only in the browser keeps server prerendering from trying to
// read config that isn't present at build time.
const isBrowser = typeof window !== "undefined";
const app = isBrowser ? (getApps().length ? getApp() : initializeApp(firebaseConfig)) : undefined;

export const auth: Auth = isBrowser ? getAuth(app!) : (undefined as unknown as Auth);
export const db: Firestore = isBrowser ? getFirestore(app!) : (undefined as unknown as Firestore);
